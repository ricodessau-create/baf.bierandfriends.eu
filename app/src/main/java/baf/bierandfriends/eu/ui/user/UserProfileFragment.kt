        ...
        binding.messageButton.setOnClickListener {
            if (targetUid.isNotEmpty()) {
                ...
                findNavController().navigate(
                    baf.bierandfriends.eu.R.id.action_userProfileFragment_to_privateChatFragment, bundle
                )
            }
        }
    ...
    private fun loadUserProfile() {
        lifecycleScope.launch {
            try {
                ...
                if (user.bio.isNotEmpty()) { binding.userBio.text = user.bio; binding.userBio.visibility = View.VISIBLE }
                if (user.location.isNotEmpty()) { binding.userLocation.text = "📍 ${user.location}"; binding.userLocation.visibility = View.VISIBLE }
                if (user.minecraftName.isNotEmpty()) { binding.userMinecraft.text = "⚔ ${user.minecraftName}"; binding.userMinecraft.visibility = View.VISIBLE }
